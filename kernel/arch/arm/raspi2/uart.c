...
int32_t uart_init(void) {
	...
	put32(MMIO_BASE+UART0_ICR, 0x7FF);
	...
	put32(MMIO_BASE+UART0_IBRD, 1);
	// Fractional part register = (.627 * 64) + 0.5 = 40.6 = ~40.
	put32(MMIO_BASE+UART0_FBRD, 40);

	// Enable FIFO & 8 bit data transmissio (1 stop bit, no parity).
	put32(MMIO_BASE+UART0_LCRH, (1 << 4) | (1 << 5) | (1 << 6));

	// Mask all interrupts.
	//put32(MMIO_BASE+UART0_IMSC, (1 << 1) | (1 << 4) | (1 << 5) | (1 << 6) | (1 << 7) |
	//		(1 << 8) | (1 << 9) | (1 << 10));
	put32(MMIO_BASE+UART0_IMSC, (1 << 4));

	// Enable UART0, receive & transfer part of UART.
	//put32(MMIO_BASE+UART0_CR, (1 << 0) | (1 << 8) | (1 << 9));
	return 0;
}

static void uart_trans(char c) {
	// Wait for UART to become ready to transmit.
	while (get32(MMIO_BASE+UART0_FR) & (1 << 5)) {}
	if(c == '\r')
		c = '\n';
